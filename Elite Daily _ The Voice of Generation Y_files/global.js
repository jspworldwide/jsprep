		...
		if ( target.hasClass('fav') || target.hasClass('unfav') ) {
			...
			function(response) {
				...
				target.fadeOut( 100, function() {
					...
					jq(this).attr('title', 'fav' == type ? 'Saved' : 'Read Later');
					jq(this).fadeIn(100);
				});
				...
				if ( 'fav' == type ) {
					...
					target.removeClass('fav icon-ed-bookmark');
					target.addClass('unfav icon-ed-bookmark-fill');

					jq('.item-list-tabs ul li#activity-favorites span').html( Number( jq('.item-list-tabs ul li#activity-favorites span').html() ) + 1 );
				} else {
					target.removeClass('unfav icon-ed-bookmark-fill');
					target.addClass('fav icon-ed-bookmark');
